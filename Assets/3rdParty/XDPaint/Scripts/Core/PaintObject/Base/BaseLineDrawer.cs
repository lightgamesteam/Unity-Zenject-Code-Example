...
namespace XDPaint.Core.PaintObject.Base
{
	public class BaseLineDrawer
    {
        ...
        public Vector2[] DrawLine(Vector2 paintUV1, Vector2 paintUV2, Triangle triangleA, Triangle triangleB, bool canRetry = true)
        {
            ...
            while (iterationsCount < IterationsMaxCount && currentTriangle.Id != _lastTriangle.Id)
            {
                ...
                if (triangle == null)
                {
                    if (canRetry)
                    {
                        return DrawLine(paintUV2, paintUV1, _lastTriangle, _firstTriangle, false);
                    }
                    break;
                }
        ...
        private float GetRatio(float totalDistanceInPixels, Texture brushTexture, float brushSize, float[] brushSizes)
        {
        ...
        public void RenderLine(Vector2[] drawPositions, Texture brushTexture, float brushSizeActual, float[] brushSizes)
        {
            ...
            for (int i = 0; i < drawPositions.Length - 1; i += 2)
            {
            ...
            var ratio = GetRatio(totalDistance, brushTexture, brushSizeActual, brushSizes);
            var verticesCount = 0;
            for (int i = 0; i < drawPositions.Length - 1; i += 2)
            {
                verticesCount += Mathf.Max(1, (int)(distances[i / 2] * ratio));
            }
            verticesCount = Mathf.Clamp(verticesCount, 1, 16384);
