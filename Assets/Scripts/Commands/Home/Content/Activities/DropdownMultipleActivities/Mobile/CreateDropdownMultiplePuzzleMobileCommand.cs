...
{
    public class CreateDropdownMultiplePuzzleMobileCommand : ICommandWithParameters
    {
        ...
        {
            var parameter = (CreateDropdownMultiplePuzzleMobileCommandSignal) signal;
        ...
        private void CreateActivityItems(CreateDropdownMultiplePuzzleMobileCommandSignal parameter)
        {
            var asset = _contentModel.GetAssetById(parameter.AssetId);
            foreach (var activityItem in asset.Puzzle)
            {
                ...
                activityView.SetActivityItemClickSource(new PuzzleAssetItemClickViewSignal(asset.Asset.Id, -1, activityItem.itemId));
        ...
        {
            headerView.SetTitle(_localizationModel.CurrentSystemTranslations[LocalizationConstants.MorePuzzleKey]);
        }
