...
public static class UIExtensions {
    ...
    public static float GetScrollToCeneter(this ScrollRect scrollRect, RectTransform target)
    {
        // The scroll rect's view's space is used to calculate scroll position
        var view = scrollRect.viewport != null ? scrollRect.viewport : scrollRect.GetComponent<RectTransform>();
    ...
    public static void SetValue(this Toggle instance, bool value, bool sendCallback)
    {
        toggleSetMethod.Invoke(instance, new object[] {value, sendCallback});
    }
    
    public static void InvokeValue(this Toggle instance, bool value, bool invoke)
    {
        toggleSetMethod.Invoke(instance, new object[] {value, false});

        if (invoke)
        {
            instance.onValueChanged.Invoke(value);
        }
    }
}